...
class MusicPlayerWidget(QWidget):
    ...
    def __initUi(self, slider=None):
        ...
        self.__timerLbl.setFont(QFont('Arial',8))
        self.__curLenLbl.setFont(QFont('Arial',8))
    ...
    def __getMediaLengthHumanFriendly(self, filename, type):
        if type == 'wav':
            Fs, samples = wavfile.read(filename)
            media_length = len(samples)/Fs
        elif type == 'mp3':
    ...
    def setMedia(self, filename, type='mp3'):
        ...
        self.__curLenLbl.setText(self.__getMediaLengthHumanFriendly(filename,type))
    ...
    def play(self):
        self.__playBtn.setIcon('icons/pause.svg')
